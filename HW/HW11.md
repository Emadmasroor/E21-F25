...
- `newtonPoly.py` uses Newton's polynomial to determine the (unique) interpolating polynomial that connects a set of points together.
...
!!! Tip
    No modifications whatsoever are needed in `newtonPoly.py` or `polyFit.py`.

!!! Tip
    You will not turn in the code for this problem.
