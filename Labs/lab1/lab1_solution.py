...
def lab1_ex4():
    ...
    while True:
        ...
        if pins != [board.A1]:
            ...
            print('first touch incorrect:', pins)
            cp.pixels.fill(RED)

        else:

            cp.pixels[0] = GREEN
            print('first touch correct!')

            pins = wait_for_touch(50)

            if pins != [board.A2]:

                print('second touch incorrect:', pins)
                cp.pixels.fill(RED)

            else:

                cp.pixels[1] = GREEN
                print('second touch correct!')

                pins = wait_for_touch(50)

                if pins != [board.A3]:

                    print('third touch incorrect:', pins)
                    cp.pixels.fill(RED)

                else:

                    cp.pixels[2] = GREEN
                    print('third touch correct!')

                    pins = wait_for_touch(50)

                    if pins != [board.A4]:

                        print('fourth touch incorrect:', pins)
                        cp.pixels.fill(RED)

                    else:

                        print('fourth touch correct!')
...
# Don't forget to uncomment it at the bottom of the file!
def lab1_ex5():
    # TODO: delete the line below, and paste in a copy
    # of all of the lines inside your lab1_ex4() function,
    # not including the one that reads "def lab1_ex4():"
    pass
